...
from ui import UserInfoDialog as UserInfoDialogui
from ui import DataRecord as DataRecordui
...
class DataRecordUI(QWidget):
    ...
    def __init__(self):
       ...
       # self.ui = DataRecordui.Ui_dataRecordUI()
        ...
        for path in excel_paths:
            ...
            for index, sheet in enumerate(sheets_file.sheets()):
                ...
                for row in range(sheet.nrows):
                    ...
                    self.userInfo['stu_grade'] = row_data[6]
                   # self.userInfo['stu_class'] = row_data[3].rsplit('-', 1)[1]
                    self.userInfo['stu_class'] = row_data[3]
    ...
    def import_images_data(self):
        ...
        for index, path in enumerate(image_paths):
            ...
            if not os.path.exists('{}/stu_{}'.format(self.datasets, stu_id)):
                text = '命名错误！'
                informativeText = '<b>文件 <font color=red>{}</font> 存在问题，数据库中没有以该图片名为工号的用户。</b>'.format(path)
    ...
    def checkToApplyUserInfo(self):
        ...
        if not self.userInfoDialog.stuIDLineEdit.hasAcceptableInput():
            self.userInfoDialog.msgLabel.setText('<font color=red>你的工号输入有误，提交失败，请检查并重试！</font>')
            ...
            self.userInfoDialog.msgLabel.setText('<font color=red>你的入职时间输入有误，提交失败，请检查并重试！</font>')
        elif not self.userInfoDialog.ClassLineEdit.hasAcceptableInput():
            self.userInfoDialog.msgLabel.setText('<font color=red>你的小组输入有误，提交失败，请检查并重试！</font>')
            ...
            self.userInfoDialog.msgLabel.setText('<font color=red>你的部门输入有误，提交失败，请检查并重试！</font>')
        elif not self.userInfoDialog.ProvinceLineEdit.hasAcceptableInput():
            self.userInfoDialog.msgLabel.setText('<font color=red>你的籍贯输入有误，提交失败，请检查并重试！</font>')
...
class UserInfoDialog(QDialog):
    ...
    def __init__(self):
        ...
        #self.ui = UserInfoDialogui.Ui_userInfoDialog()
