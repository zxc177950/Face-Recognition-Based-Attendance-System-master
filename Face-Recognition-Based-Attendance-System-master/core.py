...
class CoreUI(QMainWindow):
    ...
    def InitUI(self):
        ...
        # 选择已有考勤
        self.ChooseClasspushButton.clicked.connect(self.choose_class)
    ...
    def initDb(self):
        try:
            ...
            if not dbUserCount > 0:
                ...
                self.ChooseClasspushButton.setEnabled(True)  # 启用选择名单开关
    ...
    def save_table(self):
        ...
        self.attendance_sheet = self.class_name
        if self.class_name == '':
            text = '考勤名称为空！'
            informativeText = '<b>请输入考勤名称。</b>'
        ...
        if DataRecordUI.table_exists(cursor, self.class_name):
            text = '该表名已存在'
            informativeText = '<b>请重新输入考勤名称（可附加日期时间或序号区分）。</b>'
            CoreUI.callDialog(QMessageBox.Critical, text, informativeText, QMessageBox.Ok)
            return
        try:
            ...
            for row in range(row_num):
                stu_id = self.create_class_dialog.AddStuTable.item(row, 0).text()  # 工号
    ...
    def choose_class(self):
       self.choose_class_dialog = ChooseClassDialog()
       self.choose_class_dialog.ChoosepushButton.clicked.connect(self.choose_table)
       self.choose_class_dialog.exec()
    #保存对日常考勤修改
    def choose_table(self):
        conn, cursor = connect_to_sql()
        self.attendance_sheet = 'daliy'
        try:
            row_num = self.choose_class_dialog.AddStuTable.rowCount()
            for row in range(row_num):
                stu_id = self.choose_class_dialog.AddStuTable.item(row, 0).text()  # 工号
                cn_name = self.choose_class_dialog.AddStuTable.item(row, 1).text()  # 姓名
                branch = self.choose_class_dialog.AddStuTable.item(row,2).text()   # 部门
                group_num = self.choose_class_dialog.AddStuTable.item(row,3).text()   # 组别
                sql_judge = 'select stu_id from `daliy` where stu_id="%s"' % ( stu_id)
                if not CoreUI.check_id_exists(cursor, sql_judge):
                    insert_sql = '''INSERT INTO `daliy` (stu_id, cn_name, branch, group_num) VALUES ("%s", "%s", "%s", "%s")''' % (
                         stu_id, cn_name, branch, group_num)
                    cursor.execute(insert_sql)
                    update_user_course_count = 'UPDATE `users` SET total_course_count=total_course_count+1 WHERE stu_id="%s"' % stu_id
                    cursor.execute(update_user_course_count)
        except Exception as e:
            ...
            CoreUI.logQueue.put('Success：选择日常考勤')
        finally:
            cursor.close()
            conn.commit()
            conn.close()

        self.choose_class_dialog.close()
    ...
    #写入考勤表
    @staticmethod
    def write_sql(class_name, stu_id,cn_name, timestamp):
        ...
        try:
            if DataRecordUI.table_exists(cursor, class_name):  # 表格存在检测
                ...
                if CoreUI.check_id_exists(cursor, sql_judge):  # 人员存在确认
                   # update_sql = 'UPDATE `%s` SET attendance="%s", attendance_time="%s" WHERE stu_id="%s"' % (
                    ...
                    if timestamp <= time:
                        print('正常')
                        insert_sql = '''INSERT INTO `attendance_log` (stu_id, cn_name,attendance,attendance_time) VALUES ("%s", "%s","%s","%s")''' % (
                            stu_id, cn_name, 1, timestamp)
                    else:
                        print('迟到')
                        insert_sql = '''INSERT INTO `attendance_log` (stu_id, cn_name,attendance,attendance_time) VALUES ("%s", "%s","%s","%s")''' % (
                            stu_id, cn_name, 0, timestamp)
                    cursor.execute(insert_sql)
            ...
            logging.info('Success：：{}完成签到！数据库存储完成'.format(stu_id))
        finally:
            cursor.close()
            conn.commit()
            conn.close()
...
class TelegramBotDialog(QDialog):
    def __init__(self):
        ...
        # self.ui = TelegramBotDialogui.Ui_telegramBotDialog()
        # self.ui.setupUi(self)
    ...
    def run(self) -> None:
        ...
        while self.isRunning:
            ...
            if CoreUI.alarmQueue.qsize() > self.alarmSignalThreshold:  # 若报警信号触发超出既定计数，进行报警
                ...
                winsound.PlaySound('陌生人.wav', flags=1)
                qmsg_text = '有陌生人请注意'
            ...
            if CoreUI.attendance_queue.qsize():
                ...
                csv_path = os.path.join('./attendance_csv', '签到流水' + '.csv')
               ...
               # sql_judge = 'select stu_id from `%s` where stu_id="%s"' % (self.core_ui.class_name, stu_id)
                sql_judge = 'select stu_id from `%s` where stu_id="%s"' % (self.core_ui.attendance_sheet, stu_id)
                conn, cursor = connect_to_sql()
                # 在签到表中才会签到
                if CoreUI.check_id_exists(cursor, sql_judge):
                    ...
                    CoreUI.write_sql(self.core_ui.attendance_sheet, stu_id,zh_name, timestamp.strftime("%Y/%m/%d %H:%M:%S"))
            ...
            if self.is_haar_faceCascade:
                ...
                if self.isFaceTrackerEnabled:
                    ...
                    for rect in face_rects:
                        ...
                        shape = predictor_68(landmarks_frame, rect)
...
class CreatClassDialog(QDialog):
    ...
    def __init__(self):
        ...
        # self.ui = CreateClassui.Ui_CreateClass()
        # self.ui.setupUi(self)
        ...
        try:
            ...
            cursor.execute('SELECT stu_id, cn_name, branch, intime, group_num FROM daliy')
            conn.commit()
            dal_data = cursor.fetchall()
            # print(stu_data)
            self.print_to_table_add(dal_data)  # 输出到表格界面
        except FileNotFoundError:
            logging.error('系统找不到数据库表{}'.format(CoreUI.database))
        except Exception as e:
            print(e)
    ...
    def print_to_table_add(self, dal_data):
        while self.AddStuTable.rowCount() > 0:
            self.AddStuTable.removeRow(0)
        for row_index, row_data in enumerate(dal_data):
            self.AddStuTable.insertRow(row_index)  # 插入行
            for col_index, col_data in enumerate(row_data):  # 插入列
                self.AddStuTable.setItem(row_index, col_index, QTableWidgetItem(str(col_data)))  # 设置单元格文本
    # 增加学生
...
class ChooseClassDialog(QDialog):
    def __init__(self):
        super(ChooseClassDialog, self).__init__()
        loadUi('./ui/ChooseClass.ui', self)  # 读取UI布局
        ...
        try:
            ...
            cursor.execute('SELECT stu_id, cn_name, major, class, grade FROM users')
            conn.commit()
            stu_data = cursor.fetchall()
            # print(stu_data)
            self.print_to_table(stu_data)  # 输出到表格界面
        ...
        try:
            conn_2, cursor_2 = connect_to_sql()  # 连接数据库

            if not DataRecordUI.table_exists(cursor_2, CoreUI.database):
                raise FileNotFoundError

            cursor_2.execute('SELECT stu_id, cn_name, branch,  group_num FROM daliy')
            conn_2.commit()
            dal_data = cursor_2.fetchall()
           # print(dal_data)
            self.print_to_table_add(dal_data)  # 输出到表格界面
          #  print("1")
        except FileNotFoundError:
