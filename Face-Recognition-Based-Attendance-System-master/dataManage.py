...
from ui import DataManage as DataManageui
from ui import export_excel as export_excelui
...
class DataManageUI(QWidget):
    ...
    def __init__(self):
        ...
        #self.ui = DataManageui.Ui_dataManageUI()
        ...
        self.ExportExcelpushButton.clicked.connect(self.check_table)  # 导出考勤表

        # 考勤日志管理
        self.LogmanagepushButton.clicked.connect(self.manage_log)
    ...
    # 考勤日志
    def manage_log(self):
        self.manage_log_dialog = ManageLogDialog()
        self.manage_log_dialog.exec()
    # 数据修改提交数据库
    ...
    def initDb(self):
        try:
            ...
            if not self.isDbReady:
                ...
                self.LogmanagepushButton.setToolTip('')
                self.LogmanagepushButton.setEnabled(True)  # 启用考勤日志管理按钮
            else:
...
# 考勤日志管理
class ManageLogDialog(QDialog):
    sql_name_map = ('id''stu_id', 'cn_name', 'attendance',  'attendance_time')  # 数据库字段映射
    def __init__(self):
        super(ManageLogDialog, self).__init__()
        loadUi('./ui/Managelog.ui', self)
        self.setWindowIcon(QIcon('./icons/icon.png'))
        self.show_log.itemClicked.connect(self.enable_delete_button) #选中内容
        self.database = 'attendance_log'
        self.current_select = set()  # 选中内容存储器
        self.CellChangeButton.clicked.connect(self.modify_line)  # 启用/禁用编辑功能
        # 表头自动伸缩
        self.show_log.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.staff_pushButton.clicked.connect(self.staff_query)
        self.deleteLogButton.clicked.connect(self.dele_log)

        # 单元格编辑开关

    def modify_line(self):
        if self.CellChangeButton.text() == '禁用编辑':
            self.show_log.setEditTriggers(QAbstractItemView.NoEditTriggers)
            self.CellChangeButton.setText('启用编辑')
            self.enable_select_table()
        else:
            self.deleteLogButton.setEnabled(False)
            self.show_log.setEditTriggers(QAbstractItemView.DoubleClicked)
            self.CellChangeButton.setText('禁用编辑')
            self.enable_write_table()
    ...
    def enable_write_table(self):
        ...
        for row in range(row_count):
            self.show_log.item(row, 0).setFlags(Qt.ItemIsEnabled)
            self.show_log.item(row, 1).setFlags(Qt.ItemIsEnabled)
        self.show_log.cellChanged.connect(self.cell_change)  # 输出结束后重新关联修改监听

        # 禁用编辑时要将禁止选择的属性修改回来

    def enable_select_table(self):
        self.show_log.cellChanged.disconnect()
        row_count = self.show_log.rowCount()
        for row in range(row_count):
            self.show_log.item(row, 0).setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
            self.show_log.item(row, 1).setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
        # 一个大坑，所有的setflags枚举类型单独用都是相反的效果，只有用位运算合用才能达到理想的效果
    ...
    def cell_change(self, row, col):
        try:
            ...
            item = self.show_log.item(row, col)
            id = self.show_log.item(row, 0).text()
            print(id)
            after_change_txt = item.text()

            select_sql = 'SELECT * FROM attendance_log WHERE id=%s' % id
            cursor.execute(select_sql)
            ret = cursor.fetchall()
            if not ret:
                raise RecordNotFound
            ...
            text = '确定将原数据<font color=blue> {} </font>修改为<font color=green> {} </font> 吗？<font color=red>该操作不可逆！</font>'.format(
                before_change_txt, after_change_txt)
            informativeText = '<b>是否继续？</b>'
            ret = ManageLogDialog.callDialog(QMessageBox.Warning, text, informativeText, QMessageBox.Yes | QMessageBox.No,
                                          QMessageBox.No)

            if ret == QMessageBox.Yes:
                update_sql = 'UPDATE attendance_log SET %s="%s" WHERE id=%s' % (
                    self.sql_name_map[col], after_change_txt, id)
                cursor.execute(update_sql)
            else:
                if self.CellChangeButton.text() == '禁用编辑':
                    self.show_log.cellChanged.disconnect()  # 表格变化时禁用修改监听
                self.show_log.setItem(row, col, QTableWidgetItem(str(before_change_txt)))
                if self.CellChangeButton.text() == '禁用编辑':
                    self.enable_write_table()
                # 如果不严格限制修改操作，将无限循环递归触发单元格变动逻辑，直到允许提交
    ...
    def staff_query(self):
        select_data = dict()
        select_data['stu_id'] = self.IDlineEdit.text().strip()
        select_data['cn_name'] = self.NamelineEdit.text().strip()
        year = self.Year_lineEdit.text().strip()
        month = self.Month_lineEdit.text().strip()
        day = self.Day_lineEdit.text().strip()
        conn, cursor = DataManageUI.connect_to_sql()
        try:
            select_sql = 'SELECT * FROM attendance_log WHERE 1=1'
            for key, value in select_data.items():
                # if value is not '':
                if value != '':
                    select_sql += ' AND %s LIKE "%%%s%%"' % (key, value)
            ...
            if datetime_sql != '':
                print(datetime_sql)
                select_sql += ' AND  convert(attendance_time,datetime) LIKE "%%%s%%"' % (datetime_sql)
            print(select_sql)
            cursor.execute(select_sql)
            ret = cursor.fetchall()
            self.print_to_table(ret)  # 输出查询结果
            if not ret:
                raise RecordNotFound
            self.print_to_table(ret)
        except RecordNotFound:
            logging.warning('未查询到记录{}'.format(str(select_data)))
        except Exception as e:
            print(e)
            logging.error('读取数据库异常，无法查询到{}的用户信息'.format(str(select_data)))
        finally:
    ...
    #删除
    def dele_log(self):
        del_log = tuple(self.current_select)
        if len(del_log) == 1:
            str_del_log = '(' + str(del_log[0]) + ')'  # 元组只有一个元素的时候会多逗号
        else:
            str_del_log = str(del_log)
        text = '已选择{}条记录。从数据库中删除选中记录，<font color=red>该操作不可逆！</font>'.format(len(del_log))
        informativeText = '<b>是否继续？</b>'
        ret = ManageLogDialog.callDialog(QMessageBox.Warning, text, informativeText, QMessageBox.Yes | QMessageBox.No,
                                      QMessageBox.No)
        ...
        if ret == QMessageBox.Yes:
            ...
            del_sql = 'DELETE FROM attendance_log WHERE id IN %s' % str_del_log
            # print(del_sql)
            try:
                cursor.execute(del_sql)
            except Exception as e:
                print(e)
                cursor.close()
                logging.error('无法从数据库中删除{}'.format(del_log))
                self.deleteLogButton.setIcon(QIcon('./icons/error.png'))
            else:
                cursor.close()
                conn.commit()  # 删除手动commit提交

                text = '你已成功删除{}条记录。'.format(len(del_log))
                informativeText = '<b>请刷新数据。</b>'
                ManageLogDialog.callDialog(QMessageBox.Information, text, informativeText, QMessageBox.Ok)
                # 删除用户后重新读取数据库
                self.initDb()
                self.deleteLogButton.setIcon(QIcon('./icons/success.png'))
                self.deleteLogButton.setEnabled(False)
                self.staff_pushButton.setIcon(QIcon())
            finally:
                conn.close()
    #输出
    def print_to_table(self, log_data):
        while self.show_log.rowCount() > 0:
            self.show_log.removeRow(0)
        for row_index, row_data in enumerate(log_data):
            self.show_log.insertRow(row_index) #插入行
            for col_index, col_data in enumerate (row_data): #插入列
                self.show_log.setItem(row_index, col_index, QTableWidgetItem(str(col_data))) # 设置单元格文本
...
class ExportExcelDialog(QDialog):
    ...
    def __init__(self):
        ...
        #self.ui = export_excelui.Ui_export_excel()
